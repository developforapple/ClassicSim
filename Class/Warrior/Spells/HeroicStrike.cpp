...
void HeroicStrike::calculate_damage() {
    ...
    if (result == PhysicalAttackResult::CRITICAL) {
        ...
        add_crit_dmg(static_cast<int>(round(damage_dealt * warr->get_stats()->get_melee_ability_crit_dmg_mod())), resource_cost, 0, 145);
    } else if (result == PhysicalAttackResult::HIT) {
        warr->melee_mh_yellow_hit_effect();
        add_hit_dmg(static_cast<int>(round(damage_dealt)), resource_cost, 0, 145);
    }
}
